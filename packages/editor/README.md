...
**A phase speaks for the set that produced it, and the raw selection is not that set.** Three separate defects came from using `WorkspaceSelection`'s keys where the *canonicalized* membership was meant, so `BulkPhase`'s `planned`, `writing`, `partial` and `landed` arms carry their own `members`. Two consequences follow. An unsent plan goes **stale** when the effective membership moves under it — the order can regroup a selected issue into a `together-with` unit without the raw selection changing at all, which no reducer could notice, so `staleAgainst` is asked by the renderer where the slot leads are known, and a stale plan draws as `idle`. And a dispatched batch keeps stating **its own** count: one sent for A/B/C and left `partial` still says three after the reader has gone on to select D/E/F, because drawing the new set's count above a Resume that would write the old one is the block naming one set while acting on another.

Which phase to draw and which set it speaks for are **one decision**, taken by the caller that holds both sets; `bulkSpec` resolves nothing. Answering half of it inside the renderer is what let a heading and an off-canvas count disagree about which batch they were about. The block is drawn for a selected set **or** for a batch that has already gone out — gated on the set alone, a partial batch's Resume and Dismiss vanished the moment the reader collapsed the selection, leaving a remainder owed, held and unreachable.
