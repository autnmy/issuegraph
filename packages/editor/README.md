...
**A hold's holder is a control.** The inspector lists the selected issue's holds with `data-code` and `data-subject` mirroring layer 1's `holdLine`, and renders a `subject` — the open blocker, the claimed peer, the unready member, routinely not among the members drawn — as a `select-issue` button naming it, so the holder becomes a deep link into the one selection below rather than a name in a sentence.
